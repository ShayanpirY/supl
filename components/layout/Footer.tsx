    ...
    <footer className="mt-16 border-t border-white/10 bg-[#0a0a0a]">
      <div className="mx-auto max-w-7xl grid gap-10 py-12 px-4 sm:px-6 lg:px-8 md:grid-cols-2 lg:grid-cols-4">
        ...
        <div>
          <div className="flex items-center gap-2">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#e50914] text-xl font-black text-white shadow-[0_0_15px_rgba(229,9,20,0.5)]">
              م
            </div>
          ...
          <div className="mt-4 flex gap-2">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="flex h-9 w-9 items-center justify-center rounded-full bg-white/5 text-zinc-400 border border-white/10 transition-all duration-300 hover:border-[#e50914] hover:text-[#e50914]"
        ...
        {COLUMNS.map((col) => (
          <div key={col.title}>
            <h4 className="mb-4 font-extrabold text-white">{col.title}</h4>
            <ul className="space-y-2.5">
              {col.links.map((link) => (
                <li key={link.label}>
                  <Link
                    href={link.href}
                    className="text-sm text-zinc-400 transition-colors duration-300 hover:text-[#e50914]"
                  >
                    {link.label}
                  </Link>
                </li>
      ...
      <div className="border-t border-white/10 py-6">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <p className="mb-3 text-xs font-bold text-zinc-500">برندهای مطرح:</p>
      ...
      <div className="border-t border-white/10 bg-black/50">
        <div className="mx-auto max-w-7xl flex flex-col items-center justify-between gap-3 py-5 px-4 text-xs text-zinc-500 sm:px-6 lg:px-8 md:flex-row">
          <p>© ۱۴۰۴ مکمل — تمامی حقوق محفوظ است.</p>
          <div className="flex flex-wrap items-center gap-4">
            <span className="flex items-center gap-1.5">
              <PhoneCall className="h-3.5 w-3.5 text-[#e50914]" />
              ۰۲۱-۹۱۰۰۴۵۰۰
            </span>
            <span className="flex items-center gap-1.5">
